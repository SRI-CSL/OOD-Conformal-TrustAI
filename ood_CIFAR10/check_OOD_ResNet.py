...
def get_all_stats(net, device, stats_list, trial):
    ...
    if 'Mahalanobis' in stats_list or  'Mahalanobis1' in stats_list:
        ...
        if not os.path.exists('stats/Mahalanobis/cal_{}.npy'.format(trial)):
            temp_cal_stats = get_Mahalanobis_stats(net, device, cal_dataloader, sample_mean, precision, num_output)
            with open('stats/Mahalanobis/cal_{}.npy'.format(trial), 'wb') as f:
                np.save(f, temp_cal_stats)
        else: 
            with open('stats/Mahalanobis/cal_{}.npy'.format(trial), 'rb') as f:
                temp_cal_stats = np.load(f)

        if not os.path.exists('stats/Mahalanobis/in_{}.npy'.format(trial)):
            temp_in_stats = get_Mahalanobis_stats(net, device, in_test_dataloader, sample_mean, precision, num_output)
            with open('stats/Mahalanobis/in_{}.npy'.format(trial), 'wb') as f:
                np.save(f, temp_in_stats)
        else: 
            with open('stats/Mahalanobis/in_{}.npy'.format(trial), 'rb') as f:
                temp_in_stats = np.load(f)

        if not os.path.exists('stats/Mahalanobis/ood_{}_{}.npy'.format(opt.ood_dataset, trial)):
            temp_ood_stats = get_Mahalanobis_stats(net, device, out_test_dataloader, sample_mean, precision, num_output)
            with open('stats/Mahalanobis/ood_{}_{}.npy'.format(opt.ood_dataset, trial), 'wb') as f:
                np.save(f, temp_ood_stats)
        else: 
            with open('stats/Mahalanobis/ood_{}_{}.npy'.format(opt.ood_dataset, trial), 'rb') as f:
                temp_ood_stats = np.load(f)

        if 'Mahalanobis1' in stats_list:
            temp_cal_stats = temp_cal_stats[:, 3:5]
            temp_in_stats = temp_in_stats[:, 3:5]
            temp_ood_stats = temp_ood_stats[:, 3:5]
    ...
    if 'gram' in stats_list or 'gram4' in stats_list:
        ...
        if not os.path.exists('stats/gram/cal_{}.npy'.format(trial)):
            cal_gram_stats = get_deviations_gram(net, device, cal_dataloader, mins, maxs, range(1,11), num_output)
            with open('stats/gram/cal_{}.npy'.format(trial), 'wb') as f:
                np.save(f, cal_gram_stats)
        else: 
            with open('stats/gram/cal_{}.npy'.format(trial), 'rb') as f:
                cal_gram_stats = np.load(f)

        if not os.path.exists('stats/gram/in_{}.npy'.format(trial)):
            in_gram_stats = get_deviations_gram(net, device, in_test_dataloader, mins, maxs, range(1,11), num_output)
            with open('stats/gram/in_{}.npy'.format(trial), 'wb') as f:
                np.save(f, in_gram_stats)
        else: 
            with open('stats/gram/in_{}.npy'.format(trial), 'rb') as f:
                in_gram_stats = np.load(f)
        
        if not os.path.exists('stats/gram/ood_{}_{}.npy'.format(opt.ood_dataset, trial)):
            ood_gram_stats = get_deviations_gram(net, device, out_test_dataloader, mins, maxs, range(1,11), num_output)
            with open('stats/gram/ood_{}_{}.npy'.format(opt.ood_dataset, trial), 'wb') as f:
                np.save(f, ood_gram_stats)
        else: 
            with open('stats/gram/ood_{}_{}.npy'.format(opt.ood_dataset, trial), 'rb') as f:
...
def get_stats_ablation_Mahala(net, device, i, trial = 0):
    ...
    temp_cal_stats = temp_cal_stats[:, i:5]
    temp_in_stats = temp_in_stats[:, i:5]
    temp_ood_stats = temp_ood_stats[:, i:5]

    return temp_cal_stats, temp_in_stats, temp_ood_stats

def ablation_Mahala(net, device):
    tnr_list = []
    fpr_list = []
    for i in range(4,-1,-1):
        cal_set_stats, in_set_stats, ood_set_stats = get_stats_ablation_Mahala(net, device, i)
