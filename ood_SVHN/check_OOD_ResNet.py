# python check_OOD_ResNet.py --cuda --gpu 0 --net ./resnet_svhn.pth --n 5 --ood_dataset LSUN --proper_train_size 45000 --trials 1
...
from resnet_utils import get_intermediate_features
#get_intermediate_features_inference
from resnet_utils import sample_estimator, get_Mahalanobis_stats
from resnet_utils import get_min_max_gram, get_deviations_gram
from resnet_utils import get_energy_stats
# from resnet_utils import get_odin_stats, get_softmax_stats
# from resnet_utils import iterate_data_gradnorm
...
def get_all_stats(net, device, stats_list):
    ...
    if 'Mahalanobis' in stats_list:
        ...
        if not os.path.exists('sample_class_mean.p'):
        ...
        with open('sample_class_mean.p', 'rb') as f:
            sample_mean = pickle.load( f)
            f.close()
    
        with open('precision.p', 'rb') as f:
            precision = pickle.load( f)
            f.close()

        if not os.path.exists('stats/Mahalanobis/cal.npy'):
            temp_cal_stats = get_Mahalanobis_stats(net, device, cal_dataloader, sample_mean, precision, num_output)
            with open('stats/Mahalanobis/cal.npy', 'wb') as f:
                np.save(f, temp_cal_stats)
        else: 
            with open('stats/Mahalanobis/cal.npy', 'rb') as f:
                temp_cal_stats = np.load(f)

        if not os.path.exists('stats/Mahalanobis/in.npy'):
            temp_in_stats = get_Mahalanobis_stats(net, device, in_test_dataloader, sample_mean, precision, num_output)
            with open('stats/Mahalanobis/in.npy', 'wb') as f:
                np.save(f, temp_in_stats)
        else: 
            with open('stats/Mahalanobis/in.npy', 'rb') as f:
                temp_in_stats = np.load(f)

        if not os.path.exists('stats/Mahalanobis/ood_{}.npy'.format(opt.ood_dataset)):
            temp_ood_stats = get_Mahalanobis_stats(net, device, out_test_dataloader, sample_mean, precision, num_output)
            with open('stats/Mahalanobis/ood_{}.npy'.format(opt.ood_dataset), 'wb') as f:
                np.save(f, temp_ood_stats)
        else: 
            with open('stats/Mahalanobis/ood_{}.npy'.format(opt.ood_dataset), 'rb') as f:
    ...
    if 'gram' in stats_list or 'gram4' in stats_list or 'gram_all' in stats_list:
        if not os.path.exists('mins.p') or not os.path.exists('maxs.p'):
        ...
        with open('mins.p', 'rb') as f:
            mins = pickle.load(f)
            f.close()
        
        with open('maxs.p', 'rb') as f:
            maxs = pickle.load(f)
            f.close()
        
        if not os.path.exists('stats/gram'):
            os.mkdir('stats/gram')

        if not os.path.exists('stats/gram/cal.npy'):
            cal_gram_stats = get_deviations_gram(net, device, cal_dataloader, mins, maxs, range(1,11), num_output)
            with open('stats/gram/cal.npy', 'wb') as f:
                np.save(f, cal_gram_stats)
        else: 
            with open('stats/gram/cal.npy', 'rb') as f:
                cal_gram_stats = np.load(f)

        if not os.path.exists('stats/gram/in.npy'):
            in_gram_stats = get_deviations_gram(net, device, in_test_dataloader, mins, maxs, range(1,11), num_output)
            with open('stats/gram/in.npy', 'wb') as f:
                np.save(f, in_gram_stats)
        else: 
            with open('stats/gram/in.npy', 'rb') as f:
                in_gram_stats = np.load(f)
        
        if not os.path.exists('stats/gram/ood_{}.npy'.format(opt.ood_dataset)):
            ood_gram_stats = get_deviations_gram(net, device, out_test_dataloader, mins, maxs, range(1,11), num_output)
            with open('stats/gram/ood_{}.npy'.format(opt.ood_dataset), 'wb') as f:
                np.save(f, ood_gram_stats)
        else: 
            with open('stats/gram/ood_{}.npy'.format(opt.ood_dataset), 'rb') as f:
...
def get_false_alarm_multiple(net, device, stats_list = ['Mahalanobis']):
    ...
    with open('SVHN_ResNet_FPR.npy', 'wb') as f:
        np.save(f, fpr_list)
...
def check_OOD_multiple_test(net, device, stats_list = ['Mahalanobis']):
    ...
    for i in range(num_stats):
        ...
        if stats_list == ['Mahalanobis']:
            val_alphas[i] = temp_p_vals[int(len(temp_p_vals)*0.092)]*num_stats/(i+1)
        elif stats_list == ['gram']:
            val_alphas[i] = temp_p_vals[int(len(temp_p_vals)*0.1)]*num_stats/(i+1)
        ...
        # if 'Mahalanobis' in stats_list:
        #     val_alphas[i] = temp_p_vals[int(len(temp_p_vals)*0.10)]*num_stats/(i+1)
        # else: 
        #     val_alphas[i] = temp_p_vals[int(len(temp_p_vals)*0.10)]*num_stats/(i+1)
    # print(val_alphas)
...
def check_OOD_energy(net, device):
    ...
    plt.clf()
    plt.hist(cal_set_stats - threshold, bins = 100)
...
if __name__ == "__main__":
    ...
    if not os.path.exists('results/ResNet34_SVHN.csv'):
        with open("results/ResNet34_SVHN.csv", "w") as f:
            wf = writer(f)
    ...
    # with open("results/ResNet34_SVHN.csv", "a") as f:
    #     wf = writer(f)
    ...
    # # Note that the threshold in line 326 and 328 needs to be changed for each combination to control TPR appropriately 
    ...
    # if not os.path.exists('results/auroc_ResNet34_SVHN.csv'):
    #     with open("results/auroc_ResNet34_SVHN.csv", "w") as f:
    #         wf = writer(f)
    ...
    # with open("results/auroc_ResNet34_SVHN.csv", "a") as f:
    #     wf = writer(f)
