...
from resnet_utils import get_min_max_gram, get_deviations_gram, get_Mahalanobis_stats, sample_estimator, get_energy_stats
...
parser.add_argument('--energy_temperature', default = 100, help = 'Energy temperature')
...
net.load_state_dict(torch.load('resnet_cifar100.pth', map_location = device))
net.to(device)
net.eval()
...
def check_OOD_multiple_test(net, device, stats_list = ['Mahalanobis']):
    ...
    for i in range(num_stats):
        ...
        if 'Mahalanobis' in stats_list:
            val_alphas[i] = temp_p_vals[int(len(temp_p_vals)*0.105)]*num_stats/(i+1)
        else: 
            val_alphas[i] = temp_p_vals[int(len(temp_p_vals)*0.105)]*num_stats/(i+1)
    # print(val_alphas)
...
def plot_energy_scores(net, device):
    ...
    ctr = 0
    for i in range(int(len(ood_set_stats)/10)):
        print(ood_set_stats[ctr:ctr+10])
        ctr = ctr + 10

    plt.hist(ood_set_stats, bins = 100)
    plt.savefig('energy_hist_CIFAR100.png')
    plt.clf()
...
if __name__ == "__main__":
    ...
    if not os.path.exists('results/ResNet34_CIFAR100.csv'):
        with open("results/ResNet34_CIFAR100.csv", "w") as f:
            wf = writer(f)
    ...
    list_of_stats_list = [['Mahalanobis'], ['gram'], ['Mahalanobis', 'gram_all'] ]
